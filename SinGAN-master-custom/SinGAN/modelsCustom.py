...
class WDiscriminator(nn.Module):
    def __init__(self, opt):
        ...
        N = int(opt.nfc)
        self.head = ConvBlock(1,N,opt.ker_size,opt.padd_size,1)
...
class GeneratorConcatSkip2CleanAdd(nn.Module):
    def __init__(self, opt):
        ...
        self.is_cuda = torch.cuda.is_available()
        N = opt.nfc
        self.head = ConvBlock(1,N,opt.ker_size,opt.padd_size,1) #GenConvTransBlock(opt.nc_z,N,opt.ker_size,opt.padd_size,opt.stride)
        self.body = nn.Sequential()
        for i in range(opt.num_layer-2):
            N = int(opt.nfc/pow(2,(i+1)))
            block = ConvBlock(max(2*N,opt.min_nfc),max(N,opt.min_nfc),opt.ker_size,opt.padd_size,1)
        ...
        self.tail = nn.Sequential(
            nn.Conv3d(max(N,opt.min_nfc),1,kernel_size=opt.ker_size,stride =1,padding=opt.padd_size),
            nn.Tanh()
        )

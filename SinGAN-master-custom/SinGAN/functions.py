...
def adjust_scales2image(real_,opt):
    ...
    opt.num_scales = math.ceil((math.log(math.pow(opt.min_size / (min(real_.shape[2], real_.shape[3])), 1), opt.scale_factor_init))) + 1 #scale factor 0.75 default
    print(opt.min_size / (min(real_.shape[2], real_.shape[3])))
    print(math.pow(opt.min_size / (min(real_.shape[2], real_.shape[3])), 1))
